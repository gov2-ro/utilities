...
async def get_page_info(url, session):
    try:
        async with session.stream("GET", url, timeout=timeout) as response:  # Remove allow_redirects parameter
        ...
        if not url.startswith('http://www.') and not url.startswith('https://www.'):
            ...
            return await get_page_info(new_url, session)
...
async def process_urls(urls):
    async with httpx.AsyncClient() as session:
        for url in urls:
            ...
            page_title, og_description, etag, redirected = await get_page_info(url_with_protocol, session)
